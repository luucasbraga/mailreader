...
def json_serial(obj):
    """Converte objetos incompatíveis com JSON para tipos compatíveis."""
    if isinstance(obj, np.float32):  # Corrige o erro de float32
        return float(obj)
...
def processar_documento(pdf_content):
    ...
    # Use json.dumps com o argumento `default`
    json_data = json.dumps(documento, ensure_ascii=False, indent=4, default=json_serial)
    print(json_data)
